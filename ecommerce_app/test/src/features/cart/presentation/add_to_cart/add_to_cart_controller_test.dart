import 'package:ecommerce_app/src/features/cart/domain/item.dart';
import 'package:ecommerce_app/src/features/cart/presentation/add_to_cart/add_to_cart_controller.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mocktail/mocktail.dart';

import '../../../../mocks.dart';

void main() {
  const productId = '1';
  group('addItem', () {
    test('item added with quantity = 2, success', () async {
      // setup
      const quantity = 2;
      const item = Item(productId: productId, quantity: quantity);
      final cartService = MockCartService();
      when(() => cartService.addItem(item)).thenAnswer(
        (_) => Future.value(null),
      );
      // run & verify
      final controller = AddToCartController(cartService: cartService);
      expect(
        controller.state,
        const AsyncData(1),
      );
      controller.updateQuantity(quantity);
      expect(
        controller.state,
        const AsyncData(2),
      );
      // * if desired, use expectLater and emitsInOrder here to check that
      // * addItems emits two values
      await controller.addItem(productId);
      verify(() => cartService.addItem(item)).called(1);
      // check that quantity goes back to 1 after adding an item
      expect(
        controller.state,
        const AsyncData(1),
      );
    });

    test('item added with quantity = 2, failure', () async {
      const quantity = 2;
      const item = Item(productId: productId, quantity: quantity);
      final cartService = MockCartService();
      when(() => cartService.addItem(item))
          .thenThrow((_) => Exception('Connection failed'));
      final controller = AddToCartController(cartService: cartService);
      expect(
        controller.state,
        const AsyncData(1),
      );
      controller.updateQuantity(quantity);
      expect(
        controller.state,
        const AsyncData(2),
      );
      // * if desired, use expectLater and emitsInOrder here to check that
      // * addItems emits two values
      await controller.addItem(productId);
      verify(() => cartService.addItem(item)).called(1);
      // check that quantity goes back to 1 after adding an item
      expect(
        controller.state,
        predicate<AsyncValue<int>>(
          (value) {
            expect(value.hasError, true);
            return true;
          },
        ),
      );
    });
  });
}


/*
generated by github copilot 
late MockCartService mockCartService;
  late AddToCartController addToCartController;

  setUp(() {
    mockCartService = MockCartService();
    addToCartController = AddToCartController(cartService: mockCartService);
  });

  group('AddToCartController', () {
    test('initial state is AsyncData with value 1', () {
      expect(addToCartController.state, const AsyncData<int>(1));
    });

    test('updateQuantity updates the state with the new quantity', () {
      addToCartController.updateQuantity(5);
      expect(addToCartController.state, const AsyncData<int>(5));
    });

    test(
        'addItem sets state to AsyncLoading and then to AsyncData(1) on success',
        () async {
      const productId = '1';
      final item = Item(productId: productId, quantity: 1);

      when(() => mockCartService.addItem(item))
          .thenAnswer((_) async => Future.value());

      await addToCartController.addItem(productId);

      expect(addToCartController.state, const AsyncData<int>(1));
      verify(() => mockCartService.addItem(item)).called(1);
    });

    test('addItem sets state to AsyncError on failure', () async {
      const productId = '1';
      final item = Item(productId: productId, quantity: 1);
      final exception = Exception('Failed to add item');

      when(() => mockCartService.addItem(item)).thenThrow(exception);

      await addToCartController.addItem(productId);

      expect(addToCartController.state, isA<AsyncError<int>>());
      verify(() => mockCartService.addItem(item)).called(1);
    });
  });
}
*/
